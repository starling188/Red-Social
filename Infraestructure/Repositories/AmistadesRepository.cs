...
{
    public class AmistadesRepository : GenericRepository<Amistades> , IRepositoryAmistades
    {
        public AmistadesRepository(SocialRedContext context) : base(context) { }


        public override async Task Add(Amistades entity)
        {
            if (entity == null)
            {
        ...
        public override async Task AddList(List<Amistades> entities)
        {
            if (entities == null)
            {
        ...
        public override async Task<List<Amistades>> GetAll()
        {
            var user = await _context.Amistades.Where(c => c.IsActive).ToListAsync();
            if (user == null)
            {
        ...
        public override async Task<Amistades> GetById(int id)
        {
        ...
        public override async Task Update(Amistades entity)
        {
            if (entity == null)
            {
                throw new ArgumentNullException(nameof(entity), "El usuario no puede ser nulo");
